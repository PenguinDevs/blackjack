...
function formatGameStateForAI(gameState: BlackjackGameState): string {
  ...
  const playerCards = playerHand.cards.map(card => `${card.rank}${card.suit.charAt(0).toUpperCase()}`).join(', ')
  const dealerVisibleCardsStr = visibleDealerCards.map(card => `${card.rank}${card.suit.charAt(0).toUpperCase()}`).join(', ')
  
  // Determine if player has soft hand (ace as 11)
  const hasUsableAce = playerHand.cards.some(card => card.rank === 'A') && 
    playerHand.cards.reduce((sum, card) => sum + (card.rank === 'A' ? 1 : card.value), 0) + 10 === playerHand.value
  
  // Get dealer's upcard for strategy analysis
  const dealerUpcard = dealerVisibleCardsStr.length > 0 ? dealerVisibleCardsStr.split(', ')[0] : 'Unknown'
 ...
 */
export async function getAIRecommendation(gameState: BlackjackGameState): Promise<AIRecommendation> {
  try {
    ...
    try {
      ...
      if (!recommendation.action || !recommendation.reasoning || typeof recommendation.confidence !== 'number') {
      ...
      console.log(`✅ Gemini AI recommendation: ${recommendation.action.toUpperCase()} (${Math.round(recommendation.confidence * 100)}%)`)
      return recommendation
      
    } catch (parseError) {
      console.warn('Failed to parse Gemini AI response, falling back to basic strategy:', parseError)
